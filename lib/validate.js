...
module.exports = {
	load_params: function (taskname, xmlPath) {

		if (taskname && !is_valid_taskname(taskname)) error('Invalid taskname')
		if (xmlPath && !is_valid_xmlPath(xmlPath)) error('Invalid xml path')
	},
...
function is_valid_xmlPath(xmlPath) {
	return parse(xmlPath).ext.toLowerCase() === '.xml'
}
